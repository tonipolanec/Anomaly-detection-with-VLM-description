...
class SimpleNet(torch.nn.Module):
    ...
    def test(self, training_data, test_data, save_segmentation_images=True):
    ...
    def _evaluate(self, test_data, scores, segmentations, labels_gt, masks_gt):
        ...
        image_metrics = metrics.compute_imagewise_retrieval_metrics(
            scores, labels_gt
        )
        ...
        if len(masks_gt) > 0:
            ...
            # Persist useful thresholds to the checkpoint directory for later inference use
            try:
                thresholds_payload = {
                    "pixel_optimal_threshold": float(pixel_scores.get("optimal_threshold", 0.0)),
                    "pixel_optimal_fpr": float(pixel_scores.get("optimal_fpr", 0.0)),
                    "pixel_optimal_fnr": float(pixel_scores.get("optimal_fnr", 0.0)),
                }
                # Derive an image-level operating point using Youden's J statistic
                fpr = image_metrics.get("fpr", None)
                tpr = image_metrics.get("tpr", None)
                thr = image_metrics.get("threshold", None)
                if fpr is not None and tpr is not None and thr is not None and len(thr) == len(tpr) == len(fpr):
                    j_scores = (np.asarray(tpr) - np.asarray(fpr)).astype(np.float32)
                    best_idx = int(np.argmax(j_scores))
                    thresholds_payload["image_best_threshold_j"] = float(thr[best_idx])
                # Save beside checkpoint
                save_dir = getattr(self, "ckpt_dir", None)
                if save_dir is None:
                    save_dir = self.model_dir if hasattr(self, "model_dir") else "."
    ...
    def train(self, training_data, test_data, save_segmentation_images=True):
        ...
        for i_mepoch in range(self.meta_epochs):
            ...
            scores, segmentations, labels_gt, masks_gt = self.predict(test_data)
            auroc, full_pixel_auroc, pro = self._evaluate(test_data, scores, segmentations, labels_gt, masks_gt)
            ...
            print(f"\n----- {i_mepoch} I-AUROC:{round(auroc, 4)}(MAX:{round(best_record[0], 4)})"
                  f"  P-AUROC{round(full_pixel_auroc, 4)}(MAX:{round(best_record[1], 4)}) -----"
                  f"  PRO-AUROC{round(pro, 4)}(MAX:{round(best_record[2], 4)}) -----")

        torch.save(state_dict, ckpt_path)

        # Save segmentation images at the end of training if requested
        print(f"SAVING SEGMENTATION IMAGES")
        #if True or save_segmentation_images:
        scores, segmentations, labels_gt, masks_gt = self.predict(test_data)
        self.save_segmentation_images(test_data, segmentations, scores)
        ...
        with tqdm.tqdm(total=self.gan_epochs) as pbar:
            for i_epoch in range(self.gan_epochs):
                ...
                for data_item in input_data:
                    ...
                    noise_idxs = torch.randint(0, self.mix_noise, (true_feats.shape[0],), device=self.device)
                    noise_scales = (self.noise_std * (1.1 ** noise_idxs)).float().view(-1, 1)
                    noise = torch.randn_like(true_feats) * noise_scales
                    ...
                    # Detach and move to CPU for logging, then free GPU tensors
                    loss = loss.detach().cpu()
                    all_loss.append(loss.item())
                    all_p_true.append(p_true.cpu().item())
                    all_p_fake.append(p_fake.cpu().item())
                    del img, true_feats, fake_feats, scores, true_scores, fake_scores, noise
                    if torch.cuda.is_available() and (i_iter % 20 == 0):
                        torch.cuda.empty_cache()
                ...
                pbar_str = f"gan_epoch:{i_epoch} loss:{round(all_loss, 5)} "
    ...
    def _predict_dataloader(self, dataloader, prefix):
        """Provide anomaly scores and masks for a full dataloader without storing intermediate features."""
        _ = self.forward_modules.eval()
    ...
    def _predict(self, images):
        """Infer score and mask for a batch of images without retaining large intermediate features."""
        ...
        with torch.no_grad():
            ...
            # Do not upsample or carry features forward; only compute masks from patch scores
            masks, _ = self.anomaly_segmentor.convert_to_segmentation(patch_scores, features=None)
    ...
    def save_segmentation_images(self, data, segmentations, scores):
        print(f"\nSaving segmentation images ({len(data.dataset.data_to_iterate)})...")
